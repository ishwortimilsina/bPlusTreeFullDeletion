...
public class BTreeFile extends IndexFile implements GlobalConst {
	...
	public static float MIN_OCCUPANCY = 0.5f;
	private KeyClass _Delete(KeyClass key, RID rid, PageId currentPageId, PageId parentPageId)
		...
		{
		/*
		Since we do not know if the currentPageId is of leafPage or IndexPage
		//lets initiate a sortedPage using that currentPageId and later find out
		//if it is a leaf node or index node
		 */
		BTSortedPage currentPage = new BTSortedPage(pinPage(currentPageId), getHeaderPage().get_keyType());
		PageId nextPageId;	
		KeyClass deleteEntry;
		
		//If currentPage is Leaf node, do the following
		
		if(currentPage.getType() == NodeType.INDEX)
			{
			unpinPage(currentPageId);
			BTIndexPage currentIndexPage = new BTIndexPage(pinPage(currentPageId), getHeaderPage().get_keyType());
			
			//find out the ID child of the current page with the key provided to us
			nextPageId = currentIndexPage.getPageNoByKey(key);
			
			//unpin the page and pin it again later
			unpinPage(currentIndexPage.getCurPage());
			
			deleteEntry = _Delete(key, rid, nextPageId, currentPageId);
			
			if (deleteEntry == null)
				return null;
			
			currentIndexPage = new BTIndexPage(pinPage(currentPageId), getHeaderPage().get_keyType());
			
			//if currentIndex Page has no parent, return null
			if (parentPageId == null)
				return null;
			//if currentIndex page has a parent
			else
				{
				//Initiating the parent page for the currentIndexPage 
				BTIndexPage parentPage = new BTIndexPage (pinPage (parentPageId), getHeaderPage().get_keyType());

				//Take the first entry from the parent page
				KeyDataEntry searchEntry = parentPage.getFirst(rid);
				
				//Checking if the currentIndexPage has more space than maximum space allowed
				if (currentIndexPage.available_space() > (((MAX_SPACE - HFPage.DPFIXED) * MIN_OCCUPANCY)+ HFPage.SIZE_OF_SLOT))
					{
						//Initiating a sibling index page for the current index page for redistribution or merging
						PageId siblingIndexPageId = new PageId();
						
						/* this gives the direction (previous, no or next) of the operable sibling page
						direction = -1, sibling on the left (previous)
						direction = 0 , no sibling
						direction = +1, sibling on the right (next) 
						*/
						int direction = parentPage.getSibling(key, siblingIndexPageId);
						BTIndexPage siblingIndexPage = new BTIndexPage (pinPage(siblingIndexPageId), getHeaderPage().get_keyType());
						
						//trying to redistribute
						//if available space on current page is less than or equal to filled space in sibling page, redistribute
						if (currentIndexPage.available_space() <= ((MAX_SPACE - HFPage.DPFIXED) - siblingIndexPage.available_space()))
							siblingIndexPage.redistribute(currentIndexPage, parentPage, direction, key);
						
						else //redistribution was unsuccessful
						//************************MERGING******************************
							{
							KeyDataEntry tempEntry;
							RID delRid = new RID();
							
							// take the first entry of the sibling page
							while((tempEntry = currentIndexPage.getFirst(delRid)) != null)
								{
								//insert the first entry of the sibling page into the currentIndexPage
								siblingIndexPage.insertKey(tempEntry.key, ((IndexData) tempEntry.data).getData());
								
								//delete the record from the sibling page
								currentIndexPage.deleteSortedRecord(delRid);
								
								//repeat until there is no record (null) left on the sibling page
								}
							/*	
							 Following maintains the doubly linked list after the sibling is deleted after merging.
							 since current page will be no more, sibling has to update its prevPage and nextPage
							 for direction = -1, link has to be set with the right sided page
							 for direction = 1, link has to be set with the left sided page
							 */
								if(direction == -1)		
									{		
									//finding out the ID of next page to the sibling page and initiating that page
									PageId rightPageId = currentIndexPage.getNextPage();
									
									//link the sibling page to the next page
									siblingIndexPage.setNextPage(rightPageId);
									
									//if there is page on on the right
									if(rightPageId.pid != -1)
										{
										//initiate the rightPage
										BTIndexPage rightPage = new BTIndexPage(pinPage(rightPageId), getHeaderPage().get_keyType());
										
										//maintain the link to the left
										rightPage.setPrevPage(siblingIndexPageId);
										
										unpinPage(rightPage.getCurPage());
										}
									}
								else if (direction == 1) 
									{
									//finding out the ID of prev page to the sibling page and initiating that page
									PageId leftPageId = currentIndexPage.getPrevPage();
									
									//maintain the link to the left
									siblingIndexPage.setPrevPage(leftPageId);
									
									//if there is page on the left
									if(leftPageId.pid != -1)
										{
										//initiate the leftPage
										BTIndexPage leftPage = new BTIndexPage(pinPage(leftPageId), getHeaderPage().get_keyType());																			
										
										//maintain the link to the right
										leftPage.setNextPage(siblingIndexPageId); 
										unpinPage(leftPage.getCurPage());
										}
									}
								// Link listing operation finished							
							/*
							 * Lets find out the last entry of the sibling page after merging
							 * then locate the linking key in the parent page and pull it
							 * down from the parent page to this page
							 */
								RID lastRid = new RID();
								KeyDataEntry lastEntry = siblingIndexPage.getFirst(lastRid);
								int count=0;
								
								for (siblingIndexPage.getFirst(lastRid) ; siblingIndexPage.getNext(lastRid) != null;)
								{
									count++;
									if(count>1)
										lastEntry.key = siblingIndexPage.getNext(lastRid).key;
									else
										break;
								}
								//Last entry key of the sibling page found
								
								//Lets find the linking key on parent page
								System.out.println("SearchEntry : " + searchEntry.key);
								System.out.println("LastEntry : "+lastEntry.key);
								while (BT.keyCompare(searchEntry.key, lastEntry.key) <= 0)
									{
									searchEntry = parentPage.getNext(rid);
									}
								System.out.println("SearchEntry 1 : " + searchEntry.key);
							//pull down the key to the sibling page
							siblingIndexPage.insertKey(searchEntry.key, currentIndexPage.getLeftLink());
							//delete the key from the parent page
							parentPage.deleteKey(searchEntry.key);
							
							//unpin and then delete the currentIndexPage
							unpinPage(currentIndexPage.getCurPage());
							//freePage(currentIndexPage.getCurPage());
							
							return searchEntry.key;
							}
						//if the parent above has no entry, update the header to the current (sibling) page
						if(parentPage.getFirst(rid) == null)
							updateHeader(siblingIndexPageId);
						}
				return null;
					}
			}
		
		//If the current node is not the index, it must be leaf, so do the following
		else if (currentPage.getType() == NodeType.LEAF)
			{
			unpinPage(currentPageId); //unpin the sorted page
			
			//initiate the leafPage with the same page Id as the current page
			BTLeafPage currentLeafPage = new BTLeafPage(pinPage(currentPageId), getHeaderPage().get_keyType());
			
			//delete the entry from the leafPage using the key and rid
			currentLeafPage.delEntry(new KeyDataEntry(key, rid));
			
			//if there is no parent to the the current page
			if (parentPageId == null)
				return null;
	
			//if there is a parent to the current leaf page do the following
			else
				{
				//initiate the parent page using the parentPageId provided to this method
				BTIndexPage parentPage = new BTIndexPage (pinPage(parentPageId), getHeaderPage().get_keyType());

				//check the space available on the current leaf page
				if (currentLeafPage.available_space() > (((MAX_SPACE - HFPage.DPFIXED) * MIN_OCCUPANCY)+HFPage.SIZE_OF_SLOT)) 
					//means redistribution or merging necessary
					{
					//initiate a sibling page to the current page if there is one
					PageId siblingLeafPageId= new PageId();
					int direction = parentPage.getSibling(key, siblingLeafPageId);
					...
					BTLeafPage siblingLeafPage = new BTLeafPage(pinPage(siblingLeafPageId), getHeaderPage().get_keyType());
						
							//try redistributing and take the boolean value of the operation
					if (currentLeafPage.available_space() <= ((MAX_SPACE - HFPage.DPFIXED) - siblingLeafPage.available_space()))
						siblingLeafPage.redistribute(currentLeafPage, parentPage, direction, key);
					
					else //redistribution unsuccessful, MERGING needed
					//***************************MERFRING******************************
						{
						KeyDataEntry entry;
						...
						//get the first entry of the sibling page
						while ((entry = currentLeafPage.getFirst(mergeRid)) != null)
							{
							//insert first entry of sibling page into the current leaf page
							siblingLeafPage.insertRecord(entry);
										
							//delete the entry from the sibling page
							currentLeafPage.delEntry(entry);
							//repeat until there is no entry on the sibling page
							}
						//merging done !!!
							/*	
							 Following maintains the doubly linked list after the sibling is deleted after merging.
						...
						if(direction == -1)		
							{		
							//finding out the ID of next page to the sibling page and initiating that page
							PageId rightPageId = currentLeafPage.getNextPage();
							
							siblingLeafPage.setNextPage(rightPageId);
							
							//if there is page on the right of the current page
							if(rightPageId.pid != -1)
								{
								//initiate the rightPage
								BTLeafPage rightPage = new BTLeafPage(pinPage(rightPageId), getHeaderPage().get_keyType());
								
								//set the left link of the right page to the sibling page
								rightPage.setPrevPage(siblingLeafPageId);
								unpinPage(rightPageId);
								}								
							}
						else if (direction == 1) 
							{
							//finding out the ID of prev page to the sibling page and initiating that page
							PageId leftPageId = currentLeafPage.getPrevPage();							
							
							//maintain the link to the left
							siblingLeafPage.setPrevPage(leftPageId);
										
							//if there is page on the left of the current page
							if(leftPageId.pid != -1)	
								{
								//initiate the leftPage
								BTLeafPage leftPage = new BTLeafPage(pinPage(leftPageId), getHeaderPage().get_keyType());
								
								//set the right link of the left page to the sibling page
								leftPage.setNextPage(siblingLeafPageId);
								unpinPage(leftPageId);
								}															
							}
						...
						/*
						 * Lets find out the last entry of the sibling page after merging
						 * then locate the linking key in the parent page and delete it
						 * from the parent page to this page
						 */
						KeyDataEntry lastEntry;
						RID lastRid = new RID();
						lastEntry = siblingLeafPage.getFirst(lastRid);
						
						int count=0;
						for (siblingLeafPage.getFirst(lastRid) ; siblingLeafPage.getNext(lastRid) != null;)
						{
							count++;
							if(count>1)
								lastEntry.key = siblingLeafPage.getNext(lastRid).key;
							else
								break;
						}
						//last key entry of the sibling page found
						//lets find out the linking key on the parent page
						while (BT.keyCompare(searchEntry.key, lastEntry.key) <= 0)
							{
							searchEntry = parentPage.getNext(rid);
							}
						...
						//unpin and free the currentPage
						unpinPage(currentPageId);
						//freePage(currentPageId);
						
						return searchEntry.key;
						}
					//if there is no entry on the parent page then point to root to the sibling page
					if((parentPage.getFirst(rid)) == null)
						updateHeader(siblingLeafPageId);
					}
				}
			return null;
			}
